...
**核心流程**：
```
检查风格指南 → 4类润色 → 生成对比报告 → question工具确认 → 应用修改
```

**工作流程（7步）**：
1. 加载配置 - 读取novel-project.yaml
2. 选择章节 - 列出reviewed章节，用户选择
3. 读取/创建风格指南 ⚠️ 易遗漏
4. 执行润色 - 4个类别 ⚠️ 必须全部执行
5. 生成对比报告 - Before/After + Rationale ⚠️ 易遗漏
6. 用户确认 - 调用question工具 ⚠️ 必须使用question工具
7. 应用修改 - 更新polished列表
...
- ⚠️ 禁止只做部分润色（遗漏类别）
- ⚠️ 禁止不生成对比报告
- ⚠️ 禁止不调用question工具（直接修改）
- ⚠️ 禁止使用markdown checkbox代替question工具
- ⚠️ 禁止不说明修改理由（Rationale）
...
{
  "questions": [{
    "question": "请选择要应用的润色修改：",
    "multiple": true,
    "options": [
      {"label": "接受项 #1: [类别] 简述", "description": "详细说明"}
    ]
  }]
}
```
