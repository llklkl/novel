...
```
novels/                          # 父级目录（工作区根目录）
├── .opencode/skills/            # 共享skill（所有小说项目共用）
│   ├── novel-ideation/
│   │   └── SKILL.md             # 创意构思
│   ├── world-building/
│   │   └── SKILL.md             # 世界观构建
...
**功能:**
- 创建新项目: 在父级目录下创建新的小说项目目录
- 切换项目: 在不同小说项目间切换
- 加载当前项目: 读取配置并验证完整性
- 项目状态查看: 显示当前项目各阶段完成状态
- 列出所有项目: 显示工作区内所有小说项目

**使用方式:**
- `/skill novel-project` 后选择操作类型
- 支持创建、切换、加载、查看状态、列出项目五种模式

**项目发现:**
- 自动扫描父级目录下所有包含 `novel-project.yaml` 的子目录
- 每个子目录视为一个独立的小说项目
